...
void parse_trigger(FILE *trig_f, int nr)
{
    ...
    trig->attach_type = (cbyte)attach_type;
...
{
	struct trig_var_data *i = nullptr, *j = nullptr;
...
void trig_data_copy(trig_data *this_data, const trig_data *trg)
{
	...
	// Serai - Really really bad idea - 06/12/04
//	this_data->cmdlist = trg->cmdlist;

	CREATE(this_data->cmdlist, struct cmdlist_element, 1);
	this_data->cmdlist->cmd = str_dup(trg->cmdlist->cmd);
	i = this_data->cmdlist;
	j = trg->cmdlist;

	while(j->next)
	{
		CREATE(i->next, struct cmdlist_element, 1);
		i = i->next;
		j = j->next;
		i->cmd = str_dup(j->cmd);
	}
...
/*    struct cmdlist_element *i, *j;*/
	...
	*/
	...
	// Serai - work around for the above "memory leak" - 06/12/04
	for(struct cmdlist_element *i = this_data->cmdlist, *j = nullptr; i; i = j)
	{
		j = i->next;
		delete[] (i->cmd);
		delete[] (i);
