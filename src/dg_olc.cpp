...
{
  struct script_data *s_src = nullptr;
  struct script_data *s_dst = nullptr;
  ...
  switch (type)
  {
    case MOB_TRIGGER:
      ...
      ((char_data *)dst)->proto_script =
        ((char_data *)src)->proto_script;
     break;
    case OBJ_TRIGGER:
...
void trigedit_disp_menu(struct descriptor_data *d)
{
  ...
  if (trig->attach_type==OBJ_TRIGGER) {
    attach_type = "Objects";
    sprintbit(GET_TRIG_TYPE(trig), (const char **) otrig_types, trgtypes);
  } else if (trig->attach_type==WLD_TRIGGER) {
    attach_type = "Rooms";
    sprintbit(GET_TRIG_TYPE(trig), (const char **) wtrig_types, trgtypes);
  } else {
    attach_type = "Mobiles";
    sprintbit(GET_TRIG_TYPE(trig), (const char **) trig_types, trgtypes);
  }
...
void trigedit_disp_types(struct descriptor_data *d)
{
  ...
  sprintbit(GET_TRIG_TYPE(OLC_TRIG(d)), (const char **) types, buf1);
...
void trigedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  switch (OLC_MODE(d)) {
    ...
    case TRIGEDIT_ARGUMENT:
      OLC_TRIG(d)->arglist = (*arg?str_dup(arg):nullptr);
      OLC_VAL(d)++;
      break;
...
void trigedit_save(struct descriptor_data *d)
{
  ...
  trig_data *live_trig;
  struct cmdlist_element *cmd, *next_cmd, *j;
    ...
    while (live_trig)
    {
      if (GET_TRIG_RNUM(live_trig) == trig_rnum) {
        if (live_trig->arglist) {
          delete[] (live_trig->arglist);
          live_trig->arglist = nullptr;
        }
        if (live_trig->name) {
          delete[] (live_trig->name);
          live_trig->name = nullptr;
        }

// Serai - 06/25/04 - Hopefully this fixes our crashes
        for(cmd = live_trig->cmdlist; cmd; cmd = next_cmd)
        {
          next_cmd = cmd->next;
          delete[] (cmd->cmd);
          delete[] (cmd);
        ...
        live_trig->cmdlist = nullptr;

        if (proto->cmdlist)
        {
          CREATE(live_trig->cmdlist, struct cmdlist_element, 1);
          live_trig->cmdlist->cmd = str_dup(proto->cmdlist->cmd);
          cmd = live_trig->cmdlist;
          j = proto->cmdlist;
...
// Serai - 06/25/04 - that's a no-no, just copy it over above!
...
#ifdef CIRCLE_MAC
  sprintf(fname, "%s:%i.trg", TRG_PREFIX, zone);
#else
  sprintf(fname, "%s/%i.trg", TRG_PREFIX, zone);
#endif
