...
WCMD(do_wdoor)
{
	...
	if ((dir = search_block(direction, (const char **) dirs, FALSE)) == -1)
	{
		wld_log(room, "wdoor: invalid direction");
		return;
	}

	if ((fd = search_block(field, (const char **) door_field, FALSE)) == -1)
	{
	...
	if (fd == 0)
	{
		if (exit)
		{
			if (exit->general_description)
				delete[] (exit->general_description);
...
WCMD(do_wload)
{
	...
	if (is_abbrev(arg1, "mob"))
	{
		...
		if((*per_room && count_mobs_room(rnum, real_room(room->number)) >= atoi(per_room)) ||
			(*per_zone && count_mobs_zone(rnum, world[real_room(room->number)].zone) >= atoi(per_zone)) ||
	...
	if ((ch = get_char_by_room(room, name)))
	{
		...
		case POS_DEAD:
  		...
  		default:			/* >= POSITION SLEEPING */
			...
			{
				send_to_char("You wish that your wounds would stop BLEEDING so much!\r\n", ch);
			}
		}
