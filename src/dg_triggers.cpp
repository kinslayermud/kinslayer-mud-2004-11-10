	...
	for (ch = world[IN_ROOM(actor)].people; ch; ch = ch_next)
	{
		...
		for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next)
		{
			...
			(!GET_TRIG_NARG(t) && is_substring(GET_TRIG_ARG(t), str))))
			{
				...
				strcpy(text, str);
				half_chop(str, t->first_arg, text);
				half_chop(text, t->second_arg, text);
				half_chop(text, t->third_arg, text);
				half_chop(text, t->fourth_arg, text);
				half_chop(text, t->fifth_arg, text);


				ADD_UID_VAR(buf, t, actor, "actor", 0);
		  
				if(MOUNT(actor))
					ADD_UID_VAR(buf, t, MOUNT(actor), "mount", 0);

				
				add_var(&GET_TRIG_VARS(t), "arg", GET_TRIG_ARG(t), 0);
				add_var(&GET_TRIG_VARS(t), "first_arg", t->first_arg, 0);
				add_var(&GET_TRIG_VARS(t), "second_arg", t->second_arg, 0);
				add_var(&GET_TRIG_VARS(t), "third_arg", t->third_arg, 0);
				add_var(&GET_TRIG_VARS(t), "fourth_arg", t->fourth_arg, 0);
				add_var(&GET_TRIG_VARS(t), "fifth_arg", t->fifth_arg, 0);
