...
void
remove_event(struct event_info *eventToRemove)
{
	struct event_info *curr;

	if (event_list == eventToRemove) {
		event_list = eventToRemove->next;
	} else {
		curr = event_list;
		while (curr && (curr->next != eventToRemove))
			curr = curr->next;
