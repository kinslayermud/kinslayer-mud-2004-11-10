...
void string_add(struct descriptor_data *d, char *str)
{
  ...
  if (terminator) {
	...
	*d->str = nullptr;
    ...
    /*
     * This fix causes the editor to nullptr out empty messages -- M. Scott 
...
void show_string(struct descriptor_data *d, char *input)
{
  ...
  /* Or if we have more to show.... */
		...
		diff = (long long) d->showstr_vector[d->showstr_page + 1];
		diff -= (long long) d->showstr_vector[d->showstr_page];
