...
void read_file(void)
{
  void add_name(cbyte level, char *name);
...
"****************************************************************************\n\n");
  ...
  for (curr_level = levels; curr_level; curr_level = curr_level->next) {
    ...
    for (j = 1; j <= i; j++)
      fputc(' ', out);
    for (j = 1; (unsigned int) j <= strlen(curr_level->params->level_name); j++)
      fputc('~', out);
    fprintf(out, "\n");
	...
	if (curr_level->params->level <= COL_LEVEL) {
	  for (j = 1; (unsigned int) j <= (IMM_NSIZE - strlen(curr_name->name)); j++)
	    strcat(buf, " ");
	}
	if (curr_level->params->level > COL_LEVEL)
	  strcat(buf, "   ");
