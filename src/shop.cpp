...
/* local functions */
int read_type_list(FILE * shop_f, struct shop_buy_data * listy, int new_format, int max);
int read_list(FILE * shop_f, struct shop_buy_data * listy, int new_format, int max, int type);
...
struct obj_data *get_hash_obj_vis(char_data * ch, char *name, struct obj_data * listy);
struct obj_data *get_slide_obj_vis(char_data * ch, char *name, struct obj_data * listy);
...
int sell_price(char_data * ch, struct obj_data * obj, int shop_nr);
...
int add_to_list(struct shop_buy_data * listy, int type, int *len, int *val);
int end_read_list(struct shop_buy_data * listy, int len, int error);
...
int is_ok_char(char_data * keeper, char_data * ch, int shop_nr)
{
	...
	if ((IS_CHANNELER(ch) && NOTRADE_MAGIC_USER(shop_nr)) ||
	(IS_RANGER(ch) && NOTRADE_CLERIC(shop_nr)) ||
				            ...
				            struct obj_data * listy)
{
	...
	for (i = listy, j = 1; i && (j <= number); i = i->next_content)
		       ...
		       char_data * keeper, int shop_nr)
{
	...
	&& (carrying_weight(ch) + object_weight(obj) <= CAN_CARRY_W(ch)))
	{
		bought++;
...
struct obj_data *get_selling_obj(char_data * ch, char *name,
		            char_data * keeper, int shop_nr, int msg)
{
...
int add_to_list(struct shop_buy_data * listy, int type, int *len, int *val)
{
	if (*val >= 0)
	{
		if (*len < MAX_SHOP_OBJ)
		{
			...
			if (*val >= 0)
			{
				BUY_TYPE(listy[*len]) = *val;
				BUY_WORD(listy[(*len)++]) = 0;
			} 
...
int read_list(FILE * shop_f, struct shop_buy_data * listy, int new_format,
	          int max, int type)
{
	...
	if (new_format)
	{
		...
		do
		{
			...
			error += add_to_list(listy, type, &len, &temp);
		}	
		...
		return (end_read_list(listy, len, error));
}


int read_type_list(FILE * shop_f, struct shop_buy_data * listy,
		       int new_format, int max)
{
	...
	if (!new_format)
		return (read_list(shop_f, listy, 0, max, LIST_TRADE));
	...
	do
	{
		...
		for (index = 0, num = NOTHING; *item_types[index] != '\n'; index++)
			...
			error += add_to_list(listy, LIST_TRADE, &len, &num);
    
			if (*ptr)
				BUY_WORD(listy[len - 1]) = str_dup(ptr);
	} 
	...
	while (!done)
	{
		...
		{		/* New shop */
			...
			for (count = 0; count < temp; count++)
			{
				...
				if (BUY_TYPE(list[count]) != -1)
					SHOP_BUYWORD(top_shop, count) = BUY_WORD(list[count]);
				else
					SHOP_BUYWORD(top_shop, count) = nullptr;
			}
...
//			buf = nullptr;  // Serai - this doesn't plug it!  Commented this out.
			delete[] (buf);  /* Plug memory leak! */
		}
	}
}
