...
int perform_drop(char_data * ch, struct obj_data * obj, cbyte mode, const char *sname, sh_int RDR);
int GET_MONEY(char_data *ch);
int get_gold_in_obj(struct obj_data *containter);
void get_check_money(char_data * ch, struct obj_data * obj);
void get_from_room(char_data * ch, char *arg);
...
void perform_drop_gold(char_data * ch, int amount, cbyte mode, sh_int RDR);
void perform_put(char_data * ch, struct obj_data * obj, struct obj_data * cont);
void name_from_drinkcon(struct obj_data * obj);
void get_from_container(char_data * ch, struct obj_data * cont, char *arg, int mode);
...
void update_chest_log(char_data *ch, struct obj_data *chest, struct obj_data *obj, int type);
...
int get_gold_in_obj(struct obj_data *containter)
{
	...
	for(obj = containter->contains;obj;obj = obj->next_content)
		if(GET_OBJ_VNUM(obj) == 1100)
			gold++;
...
int GET_MONEY(char_data *ch)
{
	...
	for(obj = ch->carrying;obj;obj = obj->next_content)
	{
		...
		if(GET_OBJ_TYPE(obj) == ITEM_CONTAINER)
			gold += get_gold_in_obj(obj);
	}

	return gold;
}

void perform_give_extra(char_data *giver, char_data *receiver, struct obj_data *obj)
{
	int i = 0;
	...
	{
		if(giver->getClan(i) && receiver->getClan(i))
			break;
	}

	if (MOB_FLAGGED(receiver, MOB_AWARD) && obj->scalp.is_scalp && !IS_CORPSE(obj)
		&& i != 16 && GET_RACE(giver) != obj->scalp.race || obj->scalp.warrants[i])
	{
		sprintf(buf, "award %s 2", GET_NAME(giver));
		command_interpreter(receiver, buf);
		obj_from_char(obj);
		extract_obj(obj);
		return;
	}

	if(MOB_FLAGGED(receiver, MOB_AWARD) && isname("master", obj->name)
		      ...
		      struct obj_data * cont)
{
	...
	if ((object_weight(obj) + object_weight(cont)) > GET_OBJ_VAL(cont, 0))
...
ACMD(do_flip) 
{
	...
	sprintf(buf, "Amount of gold = %d.", GET_MONEY(ch));
	log(buf);

	if(!*arg)
	{
		send_to_char("You can't flip nothing! Try using a coin!\r\n", ch);
		return;
	}

	if(str_cmp(arg, "coin"))
	{
		send_to_char("You can't flip that!\r\n", ch);
		return;
	}

	if(GET_GOLD(ch) <= 0)
	{
		send_to_char("You don't have a coin to spare for doing that.\r\n", ch);
		return;
	}
	...
	if(flip == 1)
		...
		sprintf(buf, "You flip a coin into the air, and it lands on the ground on it's %s.\r\n", side);
		send_to_char(buf, ch);
		sprintf(buf, "A coin is flipped in the air by $n, and lands onthe %s.", side);
...
ACMD(do_diceroll)
{
	...
	sprintf(buf, "You shake the dice, and throw them onto the ground, revealing a %d and %d on your six sided dice.\r\n",
	rolla, rollb);
		...
		else
		{
			...
			{	/* put <obj> <container> */
				...
				else
				{
					...
					{
						chest_save_setup(cont, IN_ROOM(ch));
					}
				}
			} 
...
int can_take_obj(char_data * ch, struct obj_data * obj)
{
	...
	else if ((carrying_weight(ch) + object_weight(obj)) > CAN_CARRY_W(ch))
	{
...
int my_chest(char_data *ch, struct obj_data *chest)
{

	if(GET_OBJ_VNUM(chest) == 8002 && !IS_TROLLOC(ch) && GET_LEVEL(ch) < LVL_IMMORT)
		return 0;

	else if(GET_OBJ_VNUM(chest) == 1715 && !IS_HUMAN(ch) && GET_LEVEL(ch) < LVL_IMMORT)
		return 0;
				     ...
				     struct obj_data * cont, int mode)
{
	if (mode == FIND_OBJ_INV || can_take_obj(ch, obj))
	{
		...
		else if (get_otrigger(obj, ch))
		{
			...
			{
				message(ch, "A %d trying to steal objects from a CHEST?! THIS IS LOGGED!\r\n", GET_LEVEL(ch));
				return;
			}

			if(!my_chest(ch, obj))
			{
				send_to_char("It isn't from your kind... It may be haunted!\r\n", ch);
				return;
			}
...
void update_chest_log(char_data *ch, struct obj_data *chest, struct obj_data *obj, int type)
{
	...
	if (!ch || !chest || !obj || !obj->short_description)
		return;

	if(!(file = fopen(CHEST_LOG_FILE, "a+")))
	{
		mudlog("Cannot open chest log file for update.", NRM, LVL_APPR, TRUE);
		return;
	}
	...
	*(tm + 7) = '\0';
	...
	if(type == TYPE_OUT)
	{
		...
		tm, GET_NAME(ch), obj->short_description, GET_OBJ_VNUM(chest), GET_ROOM_VNUM(ch->in_room));
	}
			     ...
			     char *arg, int mode)
{
	...
	else
	{
		...
		for (obj = cont->contains; obj; obj = next_obj)
		{
			...
			if ((obj_dotmode == FIND_ALL || isname(arg, obj->name)))
			{
				found = 1;
				perform_get_from_container(ch, obj, cont, mode);
...
int perform_drop(char_data * ch, struct obj_data * obj,
		     cbyte mode, const char *sname, sh_int RDR)
{
	if (!drop_otrigger(obj, ch))
		return 0;
...
ACMD(do_drop)
{
	...
	else
	{
		...
		{
			if (!(obj = get_obj_in_list_vis(ch, arg, ch->carrying)))
				message(ch, "You don't seem to have %s %s.\r\n", AN(arg), arg); 
...

{
	...
	if (object_weight(obj) + carrying_weight(vict) > CAN_CARRY_W(vict))
	{
...
const char *already_wearing[] = {
	...
	if(GET_OBJ_VAL(obj, 3) != 11 && where == WEAR_WIELD && IS_GREYMAN(ch))
	{
		send_to_char("You know nothing of how to use that weapon.\r\n", ch);
		return;
	}

	if(GET_EQ(ch, WEAR_SHIELD) && IS_OBJ_STAT(obj, ITEM_TWO_HANDED))
	{
		message(ch, "You can't use that right now! It requires two hands!\r\n");
		return;
	}

	if(GET_EQ(ch, WEAR_WIELD) && where == WEAR_SHIELD && IS_OBJ_STAT(GET_EQ(ch, WEAR_WIELD), ITEM_TWO_HANDED))
	{
		message(ch, "Both of your hands are occupied currently.\r\n");
		return;
	}
...
ACMD(do_wield)
{
	...
	else
	{
		...
		else if (object_weight(obj) > str_app[STRENGTH_APPLY_INDEX(ch)].wield_w)
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				if(GET_OBJ_VNUM(obj) == 3000)
				{
					if(!GET_OBJ_VAL(obj, 1))
					{
						act("$p glows with a bright light as $n holds it tightly.", TRUE, ch, obj, 0, TO_ROOM);
						act("$p glows with a bright light as you hold it tightly.", TRUE, ch, obj, 0, TO_CHAR);
						GET_OBJ_TIMER(obj) = 3;
					}
				}
