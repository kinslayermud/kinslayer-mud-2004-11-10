...
struct obj_data *get_object_in_equip(char_data * ch, char *name);
...
/* Created by Tulon on 3-22-2004, saves a value from a script to a file into the /lib/scripts directory */
void save_script_value(char *command)
{

	FILE *file;

	char real_name[1000], arg[300], waste[300];

	half_chop(command, waste, command);
	half_chop(command, arg, command);

	sprintf(real_name, "%s%s", LIB_SCRIPTS, arg);

	if(!(file = fopen(real_name, "w+")))
	{
		sprintf(buf, "Error creating file %s for script saving of value %s", real_name, command);
		script_log(buf);
		return;
	}
...
int trgvar_in_room(int vnum)
	...
	for (ch = world[vnum].people; ch !=nullptr; ch = ch->next_in_room)
		i++;

	return i;
}

obj_data *get_obj_in_list(char *name, obj_data *listy)
{
    obj_data *i;
    long id;
...
/* finds room by with name.  returns nullptr if not found */
...
void do_stat_trigger(char_data *ch, trig_data *trig)
{
    ...
    sprintf(sb, "Name: '%s%s%s',  VNum: [%s%5d%s], RNum: [%5d]\r\n",
	      COLOR_YELLOW(ch, CL_NORMAL), GET_TRIG_NAME(trig), COLOR_NORMAL(ch, CL_NORMAL),
	      COLOR_GREEN(ch, CL_NORMAL), GET_TRIG_VNUM(trig), COLOR_NORMAL(ch, CL_NORMAL),
	      GET_TRIG_RNUM(trig));

    if (trig->attach_type==OBJ_TRIGGER) {
      send_to_char("Trigger Intended Assignment: Objects\r\n", ch);
      sprintbit(GET_TRIG_TYPE(trig), (const char **) otrig_types, buf);
    } else if (trig->attach_type==WLD_TRIGGER) {
      send_to_char("Trigger Intended Assignment: Rooms\r\n", ch);
      sprintbit(GET_TRIG_TYPE(trig), (const char **) wtrig_types, buf);
    } else {
      send_to_char("Trigger Intended Assignment: Mobiles\r\n", ch);
      sprintbit(GET_TRIG_TYPE(trig), (const char **) trig_types, buf);
    }
...
void script_stat (char_data *ch, struct script_data *sc)
{
	...
	for (t = TRIGGERS(sc); t; t = t->next)
	{
		sprintf(buf, "\r\n  Trigger: %s%s%s, VNum: [%s%5d%s], RNum: [%5d]\r\n",
			COLOR_YELLOW(ch, CL_NORMAL), GET_TRIG_NAME(t), COLOR_NORMAL(ch, CL_NORMAL),
			COLOR_GREEN(ch, CL_NORMAL), GET_TRIG_VNUM(t), COLOR_NORMAL(ch, CL_NORMAL),
			GET_TRIG_RNUM(t));
		...
		{
			send_to_char("  Trigger Intended Assignment: Objects\r\n", ch);
			sprintbit(GET_TRIG_TYPE(t), (const char **) otrig_types, buf1);
		}
		
		else if (t->attach_type==WLD_TRIGGER)
		{
			send_to_char("  Trigger Intended Assignment: Rooms\r\n", ch);
			sprintbit(GET_TRIG_TYPE(t), (const char **) wtrig_types, buf1);
		...
		{
			send_to_char("  Trigger Intended Assignment: Mobiles\r\n", ch);
			sprintbit(GET_TRIG_TYPE(t), (const char **) trig_types, buf1);
		}
...
int remove_var(struct trig_var_data **var_list, char *name)
{
  ...
  for (j = nullptr, i = *var_list; i && str_cmp(name, i->name);
       j = i, i = i->next);
                ...
                int type, char *var, char *field, char *subfield, char *str)
{
	...
	int num, count;
	FILE *file;
	char extra[1000], str1[1000], str2[1000], str3[1000];

	//Commands
	char *echo_cmd[]		=		{"mecho", "oecho", "wecho"};
	char *send[]			=		{"msend", "osend", "wsend"};
...
/*************************************************/
	...
	if(!*field)
	{
		if(!str_cmp(var, "pos_standing"))
			sprintf(str, "%d", POS_STANDING);

		else if(!str_cmp(var, "pos_fighting"))
			sprintf(str, "%d", POS_FIGHTING);

		else if(!str_cmp(var, "pos_sitting"))
			sprintf(str, "%d", POS_SITTING);

		else if(!str_cmp(var, "pos_resting"))
			sprintf(str, "%d", POS_RESTING);

		else if(!str_cmp(var, "pos_sleeping"))
			sprintf(str, "%d", POS_SLEEPING);

		else if(!str_cmp(var, "pos_dead"))
			sprintf(str, "%d", POS_DEAD);

		else if(!str_cmp(var, "color_normal"))
			sprintf(str, "%s", NORMAL);

		else if(!str_cmp(var, "color_bold"))
			sprintf(str, "%s", BOLD);

		else if(!str_cmp(var, "color_red"))
			sprintf(str, "%s", RED);

		else if(!str_cmp(var, "color_green"))
			sprintf(str, "%s", GREEN);

		else if(!str_cmp(var, "color_yellow"))
			sprintf(str, "%s", YELLOW);

		else if(!str_cmp(var, "color_blue"))
			sprintf(str, "%s", BLUE);

		else if(!str_cmp(var, "color_magenta"))
			sprintf(str, "%s", DARK_RED);

		else if(!str_cmp(var, "color_cyan"))
			sprintf(str, "%s", CYAN);

		else if(!str_cmp(var, "sex_male"))
			sprintf(str, "%d", SEX_MALE);

		else if(!str_cmp(var, "sex_neutral"))
			sprintf(str, "%d", SEX_NEUTRAL);

		else if(!str_cmp(var, "sex_female"))
			sprintf(str, "%d", SEX_FEMALE);

		else if(!str_cmp(var, "trink_cost"))
			sprintf(str, "3");

		else if(!str_cmp(var, "race_human"))
			sprintf(str, "%d", RACE_HUMAN);

		else if(!str_cmp(var, "race_trolloc"))
			sprintf(str, "%d", RACE_TROLLOC);

		else if(!str_cmp(var, "class_warrior"))
			sprintf(str, "%d", CLASS_WARRIOR);

		else if(!str_cmp(var, "class_thief"))
			sprintf(str, "%d", CLASS_THIEF);

		else if(!str_cmp(var, "class_ranger"))
			sprintf(str, "%d", CLASS_RANGER);

		else if(!str_cmp(var, "class_channeler"))
			sprintf(str, "%d", CLASS_CHANNELER);

		else if(!str_cmp(var, "class_fade"))
			sprintf(str, "%d", CLASS_FADE);

		else if(!str_cmp(var, "class_greyman"))
			sprintf(str, "%d", CLASS_GREYMAN);

		else if(!str_cmp(var, "class_dreadlord"))
			sprintf(str, "%d", CLASS_DREADLORD);

		else if(!str_cmp(var, "echo"))
			sprintf(str, echo_cmd[type]);

		else if(!str_cmp(var, "send"))
			sprintf(str, send[type]);

		else if(!str_cmp(var, "echoaround"))
			sprintf(str, echoaround_cmd[type]);

		else if(!str_cmp(var, "at"))
			sprintf(str, at[type]);

		else if(!str_cmp(var, "load"))
			sprintf(str, load[type]);
...
/*************************************************/
	...
	else
	{
		...
		else
		{
	       ...
	       int type, char *line, char *buf)*/
				...
				if(*field == '@')
				{
					...
					for(count = 0;*field && *field != '@';field++, count++)
					{
					...
					find_replacement(go, sc, trig, type, str1, str2, nullptr, str3);
					
					sprintf(extra, "%s%s", LIB_SCRIPTS, str3);
				
				}
				
				else
					sprintf(extra, "%s%s",LIB_SCRIPTS, field);
								
				if(!(file = fopen(extra, "r")))
					sprintf(str, "null");
		...
		if (c)
		{
			...
			if (text_processed(field, vd, str))
				return;
			...
			else if (!strn_cmp(field, "inventory_count", 15))
			{
				...
				sprintf(str, "%d", specific_inventory_count(c, atoi(extra)));
			}
			...
			{
				if((num = clanByName(subfield)) && c->getClan(num))
					sprintf(str, "%d", num);
				else
					sprintf(str, "0");
			}
				...
				sprintf(str, "%s", AFF_FLAGGED(c, AFF_NOQUIT) ? "yes" : "no");

			else if (!str_cmp(field, "council"))
				sprintf(str, "%s", PRF_FLAGGED(c, PRF_COUNCIL) ? "yes" : "no");

			else if(!str_cmp(field, "pos"))
				sprintf(str, "%d", GET_POS(c));
		
			else if (!str_cmp(field, "master_weapon"))
				sprintf(str, "%d", GET_MASTER_WEAPON(c));

			else if (!str_cmp(field, "hqp"))
				sprintf(str, "%d", GET_HIDDEN_QP(c));


			else if (!str_cmp(field, "first_item"))
				if(c->carrying)
					one_argument(c->carrying->name, str);

				else
					sprintf(str, "Nothing");

			else if (!str_cmp(field, "mount"))
				if(MOUNT(c))
				{
					if(IS_HORSE(c))
						sprintf(str, "%d", GET_MOB_VNUM(MOUNT(c)));
				}
			
				else
					sprintf(str, "%d", 0);
			
			else if (!str_cmp(field, "room_vnum"))
				sprintf(str, "%d", GET_ROOM_VNUM(IN_ROOM(c)));

			else if (!str_cmp(field, "zone"))
				sprintf(str, "%d", world[c->in_room].zone);
		  
			else if (!str_cmp(field, "rank"))
				if((num = clanByName(subfield)) && c->getClan(num))
					sprintf(str, "%d", c->getClan(num)->rank);
				else
					sprintf(str, "0");

			else if (!str_cmp(field, "race_name"))
				sprintf(str, "%s", race_name(c));

			else if (!str_cmp(field, "QP"))
				if((num = clanByName(subfield)) && c->getClan(num))
					sprintf(str, "%d", c->getClan(num)->quest_points);
				else
					sprintf(str, "0");

			else if (!str_cmp(field, "race"))
				sprintf(str, "%d", GET_RACE(c));
			...
			else if (!str_cmp(field, "room_name"))
				strcpy(str, world[IN_ROOM(c)].name);
			...
			else if (!str_cmp(field, "warning"))
				sprintf(str, "%d", GET_WARNINGS(c));

			else if (!str_cmp(field, "weave"))
				sprintf(str, "%d", GET_WP(c));

			else if (!str_cmp(field, "shadow"))
				sprintf(str, "%d", GET_SHADOW(c));

			else if (!str_cmp(field, "maxshadow"))
				sprintf(str, "%d", GET_MAX_SHADOW(c));

			else if (!str_cmp(field, "darkpoints"))
				sprintf(str, "%d", GET_DP(c));
			...
			else if (!str_cmp(field, "canbeseen"))
			{
				...
				sprinttype(GET_CLASS(c), (const char **) pc_class_types, str);
			...
			else if(!str_cmp(field, "room_sector"))
				sprintf(str, "%d", world[c->in_room].sector_type);
			...
			{
				int pos = find_eq_pos(c, nullptr, subfield);
		...
		else if (o)
		{
			if (text_processed(field, vd, str))
				...
				sprinttype(GET_OBJ_TYPE(o), (const char **) item_types, str);
		...
		else if (r)
		{
			...
			else if (!str_cmp(field, "north"))
			{
				if (r->dir_option[NORTH])
					sprintbit(r->dir_option[NORTH]->exit_info , (const char **) exit_bits, str);
			...
			else if (!str_cmp(field, "east"))
			{
				if (r->dir_option[EAST])
					sprintbit(r->dir_option[EAST]->exit_info , (const char **) exit_bits, str);
			...
			else if (!str_cmp(field, "south"))
			{
				if (r->dir_option[SOUTH])
					sprintf(str, "%s", exit_bits[r->dir_option[SOUTH]->exit_info]);
			...
			else if (!str_cmp(field, "west"))
			{
				if (r->dir_option[WEST])
					sprintbit(r->dir_option[WEST]->exit_info , (const char **) exit_bits, str);
				else
					*str = '\0';
			}
			...
			{
				if (r->dir_option[UP])
					sprintbit(r->dir_option[UP]->exit_info , (const char **) exit_bits, str);
			...
			else if (!str_cmp(field, "down"))
			{
				if (r->dir_option[DOWN])
					sprintbit(r->dir_option[DOWN]->exit_info , (const char **) exit_bits, str);
...
struct cmdlist_element *find_end(struct cmdlist_element *cl)
{
	...
	for (c = cl->next; c && c->next; c = c->next)
	{
		...
		// Serai - 06/16 - Changed a bit to handle this nullptr
		// Also, moving the script_log() so we know what script is wrong
		if(!c)
//		{
//			script_log("Possibly else end parse error.");
			return nullptr;
//		}
...
void process_die(void *go, char *cmd) {
	char_data *ch;
	char name[300];
	char spare[220];

	two_arguments(cmd, spare, name);
		...
		for(ch = character_list;ch;ch = ch->next)
			if(isname(name, GET_NAME(ch))) {
				...
				sprintf(buf, "RUH ROH! CHAR %s NOT FOUND!", name);
				script_log(buf);
				return;
}
...
int script_driver(void *go, trig_data *trig, int type, int mode)
{
	...
	{
		sprintf(buf, "Triggers recursed beyond maximum allowed depth.  VNum %d.", GET_TRIG_VNUM(trig));
		script_log(buf);
		return ret_val;
	}
	...
	cl && GET_TRIG_DEPTH(trig); cl = cl->next)
	{
		...
		else if (!strn_cmp(p, "if ", 3))
		{
			if (process_if(p + 3, go, sc, trig, type))
				...
				// Serai - 06/16 - Reports the missing end for the bug, instead of crashing.
				if ( (cl = find_else_end(trig, cl, go, sc, type)) == nullptr)
				{
					sprintf(buf, "Missing END for IF statement in trigger VNum %d.", GET_TRIG_VNUM(trig));
					script_log(buf);
					break;  // This script is dead, break out.
				}
		}
		...
		{
			// Serai - 06/28/04 - same problem as find_end() above, didnt check for nullptr
			if ( (temp = find_done(cl)) == nullptr)
			{
				sprintf(buf, "Missing DONE for WHILE statement in trigger VNum %d.", GET_TRIG_VNUM(trig));
				script_log(buf);
				break;
			}
		...
		else if (!strn_cmp("done", p, 4))
		{
			...
			type)) {
				...
				if (GET_TRIG_LOOPS(trig) >= 10000000000)
				{
...
//					delete buf;
//					buf = nullptr;
					// Serai - 06/16 - Changed to take advantage of global buffers
					sprintf(buf, "Trigger VNum %d has looped 100 times!!!", GET_TRIG_VNUM(trig));
					script_log(buf);
				}
			}
		}
		...
		{    
			...
			else if (!strn_cmp(cmd, "fvalue ", 7))
				save_script_value(cmd);
			...
			else
			{
				...
				// Serai - 07/07/04 - Do not go on if trigger purged what it was attached to!
				//         07/18/04 - Missing 'depth--;' creates havoc.  I prefer '--depth;' anyway.
				if (dg_owner_purged)
				{
					--depth;
					return (0);
...
find_done(struct cmdlist_element *cl)
{
	...
	// Serai - 06/28/04 - same problem as find_end()
	for (c = cl->next; c && c->next; c = c->next)
	{
		for (p = c->cmd; *p && isspace(*p); p++);
