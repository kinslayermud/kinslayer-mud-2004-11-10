...
/*
 * Example: ush_int buffer_opt = BUF_OVERBOOT | BUF_VERBOSE | UF_CHECK;
 */
ush_int buffer_opt = BUF_OVERBOOT;

/*
 * 1 = Check every pointer in every buffer allocated on every   
       new allocation.
 * 0 = Recommended, unless you are tracking memory corruption at 
       the time.
 *
 * NOTE: This only works under Linux.  Do not try to use with anything 
   else!
 */
 ...
 *
 * These must be at least twice as long as PULSE_BUFFER or very very ad
 * things will happen.*/
 ...
 *
 * If you change the maximum or minimum allocations, you'll probably 
   also want
 * to change the hash number.  See the 'bufhash.c' program for   
   determining a
 * decent one.  Think prime numbers. (If it crashes, you didn't give 
   arguments.)
 *
 * In demonstration of this, consider the two following tables for 11 
   and 13.
 *
 * Hash #13		Hash #11 (defaults)
 *  5:    32 ->  6	
 *  6:    64 -> 12	 6:    64 ->  9		If you notice, the 
   defaults
 *  7:   128 -> 11	 7:   128 ->  7		cause both 6 and 16  
   order
...
* 12:  4096 ->  1	12:  4096 ->  4		the range can be extended 
  from
...
*
* MAX_ALLOC - The largest buffer magnitude allowed without griping 
  about it.
* MIN_ALLOC - The lowest buffer magnitude ever handed out.  If 20 bytes 
  are
*	requested, we will allocate this size buffer anyway.
* BUFFER_HASH - The direct size->array hash mapping number. This is 
  _not_ an
*	arbitrary number. See above.
* CACHE_SIZE - How many data->buffer mappings to maintain.  Tweak to 
  suit.
*	The number seems about right (although it wouldn't hurt to 
  increase it
*	a bit) when BUFFER_MEMORY isn't used but woefully inadequate 
  otherwise.
...
static void buffer_lock(struct buf_data *buf, int type, 
				const char *func, ush_int line);
...
#define unlock_buffers()	(bufferlist_unlock(__FUNCTION__, \
						__LINE__))
#define LOCK(buf, type)	(buffer_lock((buf), (type),\
 					__FUNCTION__, __LINE__))
#define UNLOCK(buf)		(buffer_unlock((buf), __FUNCTION__,\
 					__LINE__))
 ...
 */
#define bit(a,b)	((a) & (static_cast<decltype(b)>(1) << (b)))
...
static void buffer_sanity_check(struct buf_data **check)
{
	...
	for (magnitude = 0; magnitude <= buffer_array[BUF_MAX]; magnitude++)   
	{
		...
		if (check[magnitude] && !valid_pointer(check[magnitude]) && 
	(die = TRUE))
			log("BUF: SYSERR: %s/%d head corrupted.", check == 
		
		get_buffer_head() ? "buffer" : "memory", magnitude);
		...
		for (chk = check[magnitude]; chk; chk = chk->next)
		{
			...
			if (chk->type != BT_MALLOC && valid_pointer(chk->life) && 
		(die = TRUE))
...
static int get_magnitude(ush_long y)
{
	...
	{
		log("BUF: SYSERR: Hash result %d out of range 0-%d.", number, 
		buffer_array[BUF_MAX]);
...
static struct buf_data *in_cache(const char *data)
{
	...
	for (i = 0; i < CACHE_SIZE; i++)
		if (buffer_cache[i] && buffer_cache[i]->data == data)
		{
			...
			if (buffer_opt & BUF_VERBOSE)
				log("BUF: in_cache: Cache hit %p->%p from %p. (%d/%d, miss/hit)", buffer_cache[i],
			buffer_cache[i]->data, data, buffer_cache_stats[BUFFER_CACHE_MISSES], buffer_cache_stats[BUFFER_CACHE_HITS]);
	...
	if (buffer_opt & BUF_VERBOSE)
		log("BUF: in_cache: Cache miss. (%d/%d, miss/hit)",	buffer_cache_stats[BUFFER_CACHE_MISSES], buffer_cache_stats[BUFFER_CACHE_HITS]);
...
#if BUFFER_THREADED
	pthread_join(buf_thread, nullptr);
#endif
...
void init_buffers(void)
{
	...
	find_hash();
		...
		*/
		...
		if ((error = pthread_create(&buf_thread, nullptr, buffer_thread, nullptr)) < 0) {
	...
	set_buffer_head((struct buf_data **) calloc(buffer_array[BUF_MAX] + 1, sizeof(struct buf_data *)));
	set_memory_head((struct buf_data **) calloc(buffer_array[BUF_MAX] + 1, sizeof(struct buf_data *)));
...
static void remove_buffer(struct buf_data *removeme)
{
	struct buf_data **head, *traverse, *prev = nullptr;
	int magnitude;

	if (!removeme)
	{
		log("BUF: SYSERR: nullptr buf_data given to remove_buffer.");
		return;
	}
...
static void clear_buffer(struct buf_data *clearme)
{
	if (clearme == nullptr)
	{
		log("BUF: SYSERR: clear_buffer: nullptr argument.");
		return;
	}
...
static struct buf_data *find_this_buffer(buffer *given, int type, int *searches)
{
   ...
   */
  if ((clear = in_cache(given)) != nullptr)
    return clear;
...
got_it:
  ...
  if (buffer_opt & BUF_VERBOSE)
    log("BUF: find_this_buffer: Scanning for %p, found %p->%p (%d scans).", given, clear, clear ? clear->data : nullptr, scanned);
 ...
 */
struct buf_data *detach_buffer(buffer *data, cbyte type, const char *func, const int line_n)
{
  struct buf_data *clear;
  int scanned;

  if (data == nullptr || func == nullptr) {
...
{
  if (f == nullptr) {
    log("BUF: SYSERR: free_buffer: nullptr pointer.");
    return;
...
static struct buf_data *malloc_buffer(size_t size, int type)
{
	...
	if ((new_buf->data = (char *)malloc(size + 1)) == nullptr)
...
buffer *acquire_buffer(size_t size, int type, const char *varname, const char *who, ush_int line)
{
	...
	else if ((give = find_free_buffer(size)) == nullptr)
	{
		/*
		...
		if (buffer_opt & (BUF_VERBOSE | BUF_DETAIL))
			 ...
			 * return nullptr and hope things don't crash too soon.
			 */

		if ((give = new_buffer(allocate_size, type)) == nullptr)
			return nullptr;
	}
	 ...
	 * You can call this with a nullptr parameter to have it logged at any
	 * time though.
	 */
	/*
	 * XXX: This code works but is ugly and misleading.
	 */
void show_buffers(char_data *ch, int buffer_type, int display_type)
{
	struct buf_data *disp, **head = nullptr;
	buffer *buf;
	long i, size;
...
{
	if (buf == nullptr)
		log("BUF: SYSERR: buffer_lock: buf == nullptr from %s:%d", func, line);
...
{
	if (buf == nullptr)
		log("BUF: SYSERR: buffer_unlock: buf == nullptr from %s:%d.", func, line);
...
buffer *b2b_cpy(buffer *str1, const buffer *str2)
{
	...
	{		/* Would overflow. */
		log("BUF: SYSERR: b2b_cpy(): %p(%d) => %p(%d) would overflow.", str2, size, str1, str1->req_size);
...
buffer *s2b_cpy(buffer *str1, const char *str2)
{
	...
	if (!valid_pointer(str2))
	{	/* Only on Linux with extreme paranoia. */
		log("BUF: SYSERR: s2b_cpy: Invalid source pointer %p!", str2);
		return str1;
	}
	...
	if (size > str1->req_size)
	{
		log("BUF: SYSERR: s2b_cpy(): %p(%d) => %p(%d) would overflow.", str2, size, str1, str1->req_size);
...
{
	if (!valid_pointer(str1))
	{	/* Only on Linux with extreme paranoia. */
		log("BUF: SYSERR: b2s_cpy: Invalid destination pointer %p!", str1);
...
char *b2s_cat(char *str1, const buffer *str2)
{
	if (!valid_pointer(str1))
	{	/* Only on Linux with extreme paranoia. */
		log("BUF: SYSERR: b2s_cat: Invalid destination pointer %p!", str1);
...
buffer *s2b_cat(buffer *str1, const char *str2)
{
	...
	if (!valid_pointer(str2))
	{	/* Only on Linux with extreme paranoia. */
		log("BUF: SYSERR: s2b_cat: Invalid source pointer %p!", str2);
		return str1;
	}
	...
	{	/* Overflow would occur. */
		log("BUF: SYSERR: s2b_cat(): %p(%d) + %p(%d) > %d, would overflow.", str2, from, str1, tbuf, str1->req_size);
...
buffer *b2b_cat(buffer *str1, const buffer *str2)
{
	...
	if (from + tbuf > str1->req_size)
	{
		log("BUF: SYSERR: b2b_cat(): %p(%d) + %p(%d) > %d, would overflow.", str2, from, str1, tbuf, str1->req_size);
